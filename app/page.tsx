          ...
          <div>
            ...
            <div style={{ margin: "16px auto 0", maxWidth: 320, borderRadius: 12, overflow: "hidden", border: `1px solid ${C.border}` }}>
              <div style={{ position: "relative" as const, paddingBottom: "177.78%", height: 0 }}>
                <iframe
                  src="https://www.canva.com/design/DAHMwxMssWg/lFc_6hs103M95AYAtDYy-A/watch?embed&autoplay=1&mute=1&loop=1"
          ...
          <div style={{ textAlign: "center" as const, marginBottom: 12, padding: "4px 0" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "1fr 1fr", gap: 8, marginBottom: 6 }}>
              <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 12px" }}>
                <div style={{ fontFamily: "Arial, sans-serif", fontSize: 12, color: C.textDim, lineHeight: 1.5 }}>Origin Key tokens are denominated in 1 Satoshi = 1 OKT using cbBTC (tokenized Bitcoin from Coinbase). The 7% fee is distributed proportionately to Immutable Edition Collectables and holders of OKT.</div>
              </div>
              <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 12px" }}>
                ...
                <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 8, padding: 16, marginBottom: 12 }}>
                  ...
                  <div style={{ marginBottom: 16 }}>
                    ...
                    {(galCollection === "" || !galleryData?.artists.find((a: any) => a.name === galArtist)?.collections.find((c: any) => c.name === galCollection)) && (
                      <>
                        <Input theme={C} label="New collection name" value={galCollection} onChange={setGalCollection} placeholder="What Kind of Edition" />
                        <Input theme={C} label="Collection description" value={galColDesc} onChange={setGalColDesc} placeholder="Who, what, where, when and why" />
                      </>
                    )}
                  </div>
